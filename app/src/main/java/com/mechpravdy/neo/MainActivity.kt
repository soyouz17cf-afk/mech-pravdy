...
class MainActivity : AppCompatActivity() {
    ...
    private val cameraLauncher = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) {
        appendChat("[КАМЕРА] Фото сделано (анализ отключён)")
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            cameraButton.setOnClickListener { appendChat("[ℹ] Камера (анализ будет позже)"); captureSinglePhoto() }
    ...
    private fun setStatus(text: String, color: String) = runOnUiThread { try {
        statusText.text = text
        statusDot.setBackgroundResource(when(color) {
            "green" -> R.drawable.status_dot_green; "yellow" -> R.drawable.status_dot_yellow
            "red" -> R.drawable.status_dot_red; else -> R.drawable.status_dot_gray
        })
    } catch (_: Exception) {} }
...
""".trimIndent()
    ...
    private fun showCapsuleDialog() { try {
        ...
        AlertDialog.Builder(this)
            .setCustomTitle(TextView(this).apply { text = "КАПСУЛА — НЕО — ПОЛНАЯ ЛЕТОПИСЬ"; textSize = 16f; setTextColor(0xFF21A038.toInt()); setPadding(30, 30, 30, 10); gravity = android.view.Gravity.CENTER })
            .setView(e)
    ...
    private fun captureSinglePhoto() = try {
    ...
    private fun generateToken() {
        ...
        client.newCall(Request.Builder().url(authUrl)
    ...
    private fun sendMessage() {
        ...
        if (isLocalMode) {
            ...
            client.newCall(Request.Builder().url(currentApiUrl)
                .post(body.toString().toRequestBody("application/json; charset=utf-8".toMediaType())).build()
            ).enqueue(object : Callback {
                override fun onFailure(call: Call, e: IOException) {
                    appendChat("[ERROR] Сервер недоступен: ${e.message}")
                    matrixHeader.setConnectionLost(true); setStatus("Нет связи", "red")
                }
                override fun onResponse(call: Call, response: Response) {
                    val b = response.body?.string() ?: ""
                    if (response.isSuccessful) {
                        try { val a = gson.fromJson(b, JsonObject::class.java).getAsJsonObject("message").get("content").asString; appendChat("[NEO] $a"); matrixHeader.setLocalActive(true); setStatus("Онлайн", "green") }
                        catch (e: Exception) { appendChat("[NEO] $b"); matrixHeader.setLocalActive(true); setStatus("Онлайн", "green") }
                    } else { appendChat("[ERROR] HTTP ${response.code}"); matrixHeader.setConnectionLost(true); setStatus("Ошибка", "red") }
                    response.close()
                }
            })
        } else {
            ...
            client.newCall(Request.Builder().url(currentApiUrl)
                ...
                override fun onFailure(call: Call, e: IOException) {
                    appendChat("[ERROR] ${e.message}"); matrixHeader.setConnectionLost(true); setStatus("Нет связи", "red")
                }
                override fun onResponse(call: Call, response: Response) {
                    val b = response.body?.string() ?: ""
                    if (response.isSuccessful) {
                        ...
                        matrixHeader.setNeoActive(true); setStatus("Онлайн", "green")
                    } else { appendChat("[ERROR] HTTP ${response.code}"); matrixHeader.setConnectionLost(true); setStatus("Ошибка", "red") }
                    response.close()
                }
            })
    ...
    private fun checkToken() {
        ...
        client.newCall(Request.Builder().url(apiUrlGigaChat)
            .header("Authorization", "Bearer $token")
            .post(body.toString().toRequestBody("application/json; charset=utf-8".toMediaType())).build()
        ).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) { appendChat("[ERROR] ${e.message}") }
