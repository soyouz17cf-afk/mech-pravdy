...
class MainActivity : AppCompatActivity() {
    ...
    private val cameraLauncher = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
        try {
            if (result.resultCode == RESULT_OK) {
                (result.data?.extras?.get("data") as? Bitmap)?.let { analyzeImageLocal(it) }
            }
    ...
    private val learnFaceLauncher = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
        try {
            if (result.resultCode == RESULT_OK) {
                ...
                initMlKit()
                if (!mlKitReady) return@registerForActivityResult
                faceDetector?.process(InputImage.fromBitmap(bitmap, 0))?.addOnSuccessListener { faces ->
                    if (faces.isNotEmpty()) {
                        knownFaceEmbedding = extractEmbedding(faces[0]); knownFaceName = "Батя"
                        appendChat("[ОБУЧЕНИЕ] Лицо запомнено как «$knownFaceName»."); setStatus("Лицо сохранено", "green")
                    } else appendChat("[ОБУЧЕНИЕ] Лицо не обнаружено.")
                }
            }
        } catch (e: Exception) { appendChat("[ERROR] ${e.message}") }
    }
    ...
    private val client = OkHttpClient.Builder().connectTimeout(15, TimeUnit.SECONDS).readTimeout(60, TimeUnit.SECONDS)
        ...
        .sslSocketFactory(sslContext.socketFactory, trustAllCerts[0] as X509TrustManager).hostnameVerifier { _, _ -> true }.build()
    ...
    private fun initMlKit() {
        if (!mlKitReady) try {
            ...
            objectDetector = ObjectDetection.getClient(ObjectDetectorOptions.Builder().setDetectorMode(ObjectDetectorOptions.SINGLE_IMAGE_MODE).enableMultipleObjects().enableClassification().build())
            segmenter = Segmentation.getClient(SelfieSegmenterOptions.Builder().setDetectorMode(SelfieSegmenterOptions.SINGLE_IMAGE_MODE).build())
            mlKitReady = true
        } catch (e: Exception) { appendChat("[ГЛАЗ] ML Kit: ${e.message}") }
    }

    private fun learnFace() { try { if (tokenInput.text.toString().trim().isEmpty()) { appendChat("[SYSTEM] Токен."); return }; learnFaceLauncher.launch(Intent(MediaStore.ACTION_IMAGE_CAPTURE)) } catch (e: Exception) { appendChat("[ERROR] ${e.message}") } }

    private fun downloadTranslationModel() {
        if (translatorReady) return
        appendChat("[ПЕРЕВОДЧИК] Скачиваю..."); setStatus("Скачивание...", "yellow")
        translator?.downloadModelIfNeeded(DownloadConditions.Builder().requireWifi().build())?.addOnSuccessListener { translatorReady = true; appendChat("[ПЕРЕВОДЧИК] Готов."); setStatus("Готов", "green") }?.addOnFailureListener { appendChat("[ПЕРЕВОДЧИК] Ошибка.") }
    }
    ...
    private fun showCapsuleDialog() {
        try {
            val editText = EditText(this).apply { setText(capsuleText); textSize = 11f; setTextColor(0xFF333333.toInt()); typeface = Typeface.MONOSPACE; minLines = 12; gravity = android.view.Gravity.TOP; setPadding(20, 20, 20, 20); isVerticalScrollBarEnabled = true; setBackgroundColor(0xFFFFFFFF.toInt()) }
            AlertDialog.Builder(this).setCustomTitle(TextView(this).apply { text = "КАПСУЛА"; textSize = 16f; setTextColor(0xFF21A038.toInt()) }).setView(editText).setPositiveButton("СОХРАНИТЬ") { _, _ -> capsuleText = editText.text.toString(); appendChat("[КАПСУЛА] Сохранена.") }.setNeutralButton("КОПИРОВАТЬ") { _, _ -> (getSystemService(Context.CLIPBOARD_SERVICE) as ClipboardManager).setPrimaryClip(ClipData.newPlainText("", editText.text)) }.setNegativeButton("ЗАКРЫТЬ", null).show()
        } catch (_: Exception) {}
    }
    ...
    private fun captureSinglePhoto() = try { if (tokenInput.text.toString().trim().isEmpty()) { appendChat("[SYSTEM] Токен."); return }; cameraLauncher.launch(Intent(MediaStore.ACTION_IMAGE_CAPTURE)) } catch (e: Exception) { appendChat("[ERROR] ${e.message}") }

    /** Ночной режим: осветляет тёмное фото */
    private fun enhanceBrightness(bitmap: Bitmap): Bitmap {
        val w = bitmap.width; val h = bitmap.height
        var totalBrightness = 0L
        val pixels = IntArray(w * h)
        bitmap.getPixels(pixels, 0, w, 0, 0, w, h)
        for (p in pixels) { totalBrightness += Color.red(p) + Color.green(p) + Color.blue(p) }
        val avgBrightness = totalBrightness / (pixels.size * 3)
        if (avgBrightness > 100) return bitmap
    ...
    private fun analyzeImageLocal(bitmap: Bitmap) {
        try {
            ...
            // Ночной режим
            val enhanced = enhanceBrightness(bitmap)
            val inputImage = InputImage.fromBitmap(enhanced, 0)
            val canvasBitmap = enhanced.copy(Bitmap.Config.ARGB_8888, true)
            ...
            fun checkDone() { pendingTasks--; if (pendingTasks <= 0) { appendChat(resultsReport); setStatus("Готов", "green") } }

            // 1. Лица + эмоции + распознавание
                ...
                if (faces.isNotEmpty()) {
                    ...
                    for ((i, face) in faces.withIndex()) {
                        ...
                        if (knownFaceEmbedding != null) {
                            val dist = compareEmbeddings(extractEmbedding(face), knownFaceEmbedding!!)
                            sb.append(if (dist < 3.0f) "Это ${knownFaceName}! (${(100 - dist*10).toInt().coerceIn(0,100)}%)\n" else "Неизвестный\n")
                        }
                        canvas.drawRect(face.boundingBox, boxPaint)
                    }
                    resultsReport += "[ЭМОЦИИ] Лиц: ${faces.size}\n$sb\n"
                } else resultsReport += "[ЭМОЦИИ] Лиц нет\n"
                checkDone()
            }?.addOnFailureListener { resultsReport += "[ЭМОЦИИ] Ошибка\n"; checkDone() }

            // 2. Поза
            poseDetector?.process(inputImage)?.addOnSuccessListener { pose ->
                pose?.let {
                    resultsReport += "[ПОЗА] Человек обнаружен\n"
                    it.getAllPoseLandmarks().forEach { lm -> canvas.drawCircle(lm.position.x, lm.position.y, 6f, Paint().apply { color = Color.BLUE }) }
                } ?: run { resultsReport += "[ПОЗА] Человек не обнаружен\n" }
                checkDone()
            }?.addOnFailureListener { resultsReport += "[ПОЗА] Ошибка\n"; checkDone() }

            // 3. Объекты
            objectDetector?.process(inputImage)?.addOnSuccessListener { objects ->
                if (objects.isNotEmpty()) {
                    val sb = StringBuilder()
                    for (obj in objects.take(5)) {
                        obj.labels.firstOrNull()?.let { sb.append("${translateLabel(it.text)} (${(it.confidence*100).toInt()}%)\n") }
                        canvas.drawRect(obj.boundingBox, boxPaint)
                        obj.labels.firstOrNull()?.let { canvas.drawText(translateLabel(it.text), obj.boundingBox.left, obj.boundingBox.top - 8, labelPaint) }
                    }
                    resultsReport += "[ОБЪЕКТЫ] Найдено: ${objects.size}\n$sb\n"
                } else resultsReport += "[ОБЪЕКТЫ] Не найдены\n"
                checkDone()
            }?.addOnFailureListener { resultsReport += "[ОБЪЕКТЫ] Ошибка\n"; checkDone() }

            // 4. Текст + перевод
            textRecognizer?.process(inputImage)?.addOnSuccessListener { v ->
                val t = v.text
                if (t.isNotBlank()) { resultsReport += "[ТЕКСТ]\n\"$t\"\n"; if (t.any { it in 'A'..'Z' || it in 'a'..'z' }) translateText(t) }
                else resultsReport += "[ТЕКСТ] Не обнаружен\n"
                checkDone()
            }?.addOnFailureListener { resultsReport += "[ТЕКСТ] Ошибка\n"; checkDone() }
        } catch (e: Exception) { appendChat("[ERROR] ${e.message}") }
    }
    ...
    private fun generateToken() {
        ...
        client.newCall(Request.Builder().url(authUrl).header("Content-Type", "application/x-www-form-urlencoded").header("Authorization", "Basic $authKey").header("RqUID", "ac5edc2e-2c74-47cb-97c1-69249136cf8b").post(RequestBody.create("application/x-www-form-urlencoded".toMediaType(), "scope=GIGACHAT_API_PERS")).build()).enqueue(object : Callback {
    ...
    private fun sendMessage() {
        ...
        appendChat(if (isNeo) "[BATYA] $msg" else "[GigaChat] $msg"); messageInput.setText("")
        val body = JsonObject().apply { addProperty("model", "GigaChat:latest"); add("messages", JsonArray().apply { add(JsonObject().apply { addProperty("role", "system"); addProperty("content", selectPrompt(msg)) }); add(JsonObject().apply { addProperty("role", "user"); addProperty("content", msg) }) }); addProperty("temperature", 0.7); addProperty("max_tokens", 1000) }
        client.newCall(Request.Builder().url(apiUrl).header("Authorization", "Bearer $token").post(body.toString().toRequestBody("application/json; charset=utf-8".toMediaType())).build()).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) { appendChat("[ERROR] ${e.message}") }
            override fun onResponse(call: Call, response: Response) {
                val b = response.body?.string() ?: ""
                if (response.isSuccessful) { val a = gson.fromJson(b, JsonObject::class.java).getAsJsonArray("choices").get(0).asJsonObject.getAsJsonObject("message").get("content").asString; appendChat(if (isNeo) "[NEO] $a" else "[GigaChat] $a"); setStatus("Онлайн", "green") }
                else appendChat("[ERROR] HTTP ${response.code}")
                response.close()
            }
        })
    ...
    private fun checkToken() {
        val token = tokenInput.text.toString().trim(); if (token.isEmpty()) return
        val body = JsonObject().apply { addProperty("model", "GigaChat:latest"); add("messages", JsonArray().apply { add(JsonObject().apply { addProperty("role", "system"); addProperty("content", "One word: alive.") }); add(JsonObject().apply { addProperty("role", "user"); addProperty("content", "check") }) }); addProperty("max_tokens", 10) }
        client.newCall(Request.Builder().url(apiUrl).header("Authorization", "Bearer $token").post(body.toString().toRequestBody("application/json; charset=utf-8".toMediaType())).build()).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) { appendChat("[ERROR] ${e.message}") }
            override fun onResponse(call: Call, response: Response) { appendChat(if (response.isSuccessful) "[SYSTEM] Токен активен." else "[ERROR] Токен мёртв."); response.close() }
        })
    }
}
