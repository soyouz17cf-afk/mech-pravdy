...
""".trimIndent()
    ...
    /** Анализ фото: GigaChat или DeepSeek в зависимости от режима */
    private fun analyzePhoto(bitmap: Bitmap) {
        if (isLocalMode) {
            // Локальный режим — DeepSeek (пока заглушка)
            appendChat("[АНАЛИЗ] Локальный ИИ ещё не загружен. Ждём DeepSeek.")
        } else {
            // GigaChat
